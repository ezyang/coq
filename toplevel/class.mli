...
(** [try_add_new_coercion_with_target ref s src tg] declares [ref] as a coercion
   from [src] to [tg] *)
val try_add_new_coercion_with_target : global_reference -> locality -> polymorphic ->
...
val try_add_new_coercion : global_reference -> locality -> polymorphic -> unit
...
(** [try_add_new_coercion_subclass cst s] expects that [cst] denotes a
   ...
   ["Id_cst_tg"] *)
val try_add_new_coercion_subclass : cl_typ -> locality -> polymorphic -> unit
...
val try_add_new_coercion_with_source : global_reference -> locality -> polymorphic ->
  source:cl_typ -> unit
...
val try_add_new_identity_coercion : identifier -> locality -> polymorphic ->
  source:cl_typ -> target:cl_typ -> unit

val add_coercion_hook : polymorphic -> unit Tacexpr.declaration_hook

val add_subclass_hook : polymorphic -> unit Tacexpr.declaration_hook

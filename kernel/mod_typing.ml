    ...
    try
      ...
      match idl with [] ->
		...
		| Undef _ | OpaqueDef _ ->
		    ...
		    let cst =
		      Constraint.union (ContextSet.constraints cst1) cst3
		    in
		...
		| Def cs ->
		    ...
		    let cst = 
		      if cb.const_polymorphic then cst1
		      else Constraint.union (Context.constraints cb.const_universes) cst1 in
...
let rec struct_expr_constraints cst = function
  ...
  | SEBwith(meb,With_definition_body(_,cb))->
      struct_expr_constraints
      (* FIXME *)
        (Univ.Constraint.union (Context.constraints cb.const_universes) cst) meb
  | SEBwith(meb,With_module_body(_,_))->

...
let check_smaller_expr g strict (u,n) (v,m) =
  (n <= m && check_smaller g strict u v) ||
  (strict && n < m && check_smaller g false u v)
...
let check_leq g u v =
  ...
  | _ -> Huniv.for_all (fun ul -> exists_bigger g false ul v) u
...
let hcons_universe_context = Context.hcons
...
let pr_universe_context = Context.pr

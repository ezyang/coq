...
let bigint_of_int31 =
  let rec args_parsing args cur =
    match args with
      | [] -> cur
      | (GRef (_,b,_))::l when b = int31_0 -> args_parsing l (mult_2 cur)
      | (GRef (_,b,_))::l when b = int31_1 -> args_parsing l (add_1 (mult_2 cur))
      | _ -> raise Non_closed
  in
  function
  | GApp (_, GRef (_, c, _), args) when c=int31_construct -> args_parsing args zero
  | _ -> raise Non_closed
...
let bigint_of_word =
  let rec get_height rc =
    match rc with
    | GApp (_,GRef(_,c,_), [_;lft;rght]) when c = zn2z_WW ->
      1+max (get_height lft) (get_height rght)
    | _ -> 0
  in
  let rec transform hght rc =
    match rc with
    | GApp (_,GRef(_,c,_),_) when c = zn2z_W0-> zero
    | GApp (_,GRef(_,c,_), [_;lft;rght]) when c=zn2z_WW->
...
let bigint_of_bigZ = function
  | GApp (_, GRef(_,c,_), [one_arg]) when c = bigZ_pos -> bigint_of_bigN one_arg
  | GApp (_, GRef(_,c,_), [one_arg]) when c = bigZ_neg ->
...
let uninterp_bigQ rc =
  try match rc with
    | GApp (_, GRef(_,c,_), [one_arg]) when c = bigQ_z ->
